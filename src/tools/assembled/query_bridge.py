...
class CursorQueryBridge:
    ...
    def build_messages(self, user_query: str) -> list[dict[str, str]]:
        system_prompt = load_internal_prompt(
...
def _rewrite_tokens(query: str) -> tuple[str, ...]:
    bad_tokens = {
        "the", "and", "for", "in", "to", "of", "with", "a", "an", "is", "this", "that",
        "py", "json", "md", "txt", "yaml", "yml", "ini", "toml", "sh", "bash", "js", "ts", "html", "css"
    }
    tokens: set[str] = {
        token
        for token in re.findall(r"[A-Za-z_][A-Za-z0-9_]*", query)
        if len(token) >= 2 and token.lower() not in bad_tokens
    }
    ...
    for phrase in chinese_phrases:
        ...
        try:
            ...
            tokens.update(word for word in jieba.lcut(phrase) if len(word) >= 2 and word.lower() not in bad_tokens)
        except ImportError:
            tokens.update(
                phrase[index:index + 2]
                for index in range(max(0, len(phrase) - 1))
                if phrase[index:index + 2].lower() not in bad_tokens
            )

    for chinese, aliases in _QUERY_ALIASES.items():
        if chinese in query or chinese in tokens:
            tokens.update(alias for alias in aliases if alias.lower() not in bad_tokens)

    # Final filter just in case
    filtered = {t for t in tokens if t.lower() not in bad_tokens}

    return tuple(sorted(filtered, key=lambda token: (len(token), token))[:32])
