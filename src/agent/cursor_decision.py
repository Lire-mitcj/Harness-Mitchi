...
from src.agent.cursor_contracts import ContextPack, Decision, InterHint
from src.agent.cursor_inter_llm import _strip_json_fence
...
_DECISION_PROMPT_PATH = (
    Path(__file__).resolve().parent.parent.parent / "prompts" / "decision_prompt.md"
)


@lru_cache(maxsize=1)
def _decision_system_prompt(schema: str) -> str:
    prompt = _DECISION_PROMPT_PATH.read_text(encoding="utf-8").strip()
    ...
    def build_messages(
        ...
        *,
        ...
        for window in context_pack.windows:
            ...
            context.append(
                f"<file path=\"{window.file}\" lines=\"{window.start_line}-"
