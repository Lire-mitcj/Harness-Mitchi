...
from src.agent.cursor_contracts import ValidationDecision, ValidationResult
...
class CursorValidator:
    ...
    async def validate(
        ...
        *,
        ...
        execution_result = await self.validate_execution()
    ...
    async def validate_execution(self) -> dict[str, object]:
        try:
    ...
    *,
    ...
    for path in _iter_schema_files(project_root):
        ...
        if rel == target_file:
            continue
        try:
            view_schemas.update(_view_schemas_from_content(
